...
## Summary
- 267 nodes · 478 edges · 19 communities (16 shown, 3 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 4 edges (avg confidence: 0.65)
- Token cost: 737 input · 1,506 output

## Graph Freshness
- Built from commit: `20b9f840`
...
- `registerRoutes()` --indirect_call--> `verifyJWT()`  [INFERRED]
  server/routes.js → server/auth.js
- `registerRoutes()` --indirect_call--> `isAdmin()`  [INFERRED]
  server/routes.js → server/auth.js
...
Cohesion: 0.10
Nodes (24): ADMIN_EMAILS, App(), AppInner(), DesktopNav(), isAdmin(), NAV_ITEMS, LoginPage(), SignupPage() (+16 more)
...
Cohesion: 0.16
Nodes (19): ADMIN_EMAILS, isAdmin(), isAdminEmail(), verifyJWT(), app, __dirname, DIST, ROOT (+11 more)
...
- **Why does `registerRoutes()` connect `Server Authentication Logic` to `Vocabulary Lookup Tool`?**
